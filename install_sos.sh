...
APPIMAGE_SRC="${1:-${SCRIPT_DIR}/${APPIMAGE_NAME}}"
...
if [[ ! -f "$APPIMAGE_SRC" ]]; then
  ...
  echo "Usage: sudo ./install_sos.sh /path/to/${APPIMAGE_NAME}" >&2
  exit 1
fi

if [[ $EUID -ne 0 ]]; then
  echo "Please run as root: sudo ./install_sos.sh /path/to/${APPIMAGE_NAME}" >&2
  exit 1
fi
